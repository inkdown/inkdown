...
export class UploadQueue extends Events {
    ...
    enqueue(event: FileChangeEvent, priority: UploadPriority = 'normal'): void {
        ...
        if (existingIndex !== -1) {
            ...
            this.logger.debug(
                `Updated queued item: ${event.path} (priority: ${existing.priority})`,
            );
        } else {
            // Add new item
            this.queue.push({
    ...
    private async processQueue(): Promise<void> {
        ...
        this.logger.debug(
            `processQueue: ${readyItems.length} items ready, ${this.queue.length} total in queue`,
        );
        ...
        for (const item of readyItems) {
            ...
            try {
                this.logger.debug(
                    `Triggering upload for: ${item.event.path} (priority: ${item.priority})`,
                );
                ...
                this.logger.error(
                    `Unexpected error processing queue item: ${item.event.path}`,
                    error,
                );
    ...
    markFailure(path: string, error: Error): void {
        ...
        if (item.attempts >= this.MAX_RETRIES) {
            ...
            const delay = 2 ** item.attempts * this.BASE_DELAY_MS;
